...
copy_component_manifests() {
    ...
    if [[ -z "${readme_path_pattern_for_replacement}" ]]; then
...
for component in {backend,frontend,controller}; do
    # TODO: Update this to point to the applications directory post-GA, see the following for the current status
    ...
    copy_component_manifests "workspaces/${component}/manifests/kustomize/" \
        "experimental/notebooks-v2/${component}/upstream" ""
done

commit_changes "$MANIFESTS_DIRECTORY" "Update ${REPOSITORY_NAME} manifests from ${COMMIT}" \
  "experimental/notebooks-v2"
